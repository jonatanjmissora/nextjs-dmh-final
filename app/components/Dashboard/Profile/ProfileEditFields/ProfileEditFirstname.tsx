...
import { FirstnameType, UserDataTypes } from "@/app/types/user.types";
import { firstnameEditSchema } from "@/app/schema/userEdit.schema";
...
export default function ProfileEditFirstname({ token, accountId, user, fieldLabel, fieldValue, editing, setEditing }: ProfileEditFieldProps) {
  ...
  const {
    ...
    setFocus,
  } = useForm<FirstnameType>({
    resolver: yupResolver(firstnameEditSchema),
  })
